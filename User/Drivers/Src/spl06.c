#include "spl06.h"
#include "app.h"
// static osSemaphoreId_t sem_i2c_dma;
static spl06_calib_struct spl06_calib;

// static void spl06_write_reg(uint8_t reg, uint8_t data);
// static void spl06_read_data(uint8_t reg, uint8_t *buf, uint16_t size);
static void spl06_read_id(I2C_HandleTypeDef *hi2cx, spl06_data_struct *spl06_data);
static void spl06_write_data(I2C_HandleTypeDef *hi2cx, uint8_t reg, uint8_t data, spl06_data_struct *spl06_data);
static void spl06_read_data(I2C_HandleTypeDef *hi2cx, uint8_t reg, uint8_t *buf, uint16_t size, spl06_data_struct *spl06_data);
static void spl06_read_pressure_raw(I2C_HandleTypeDef *hi2cx, spl06_data_struct *spl06_data);
static void spl06_read_temperature_raw(I2C_HandleTypeDef *hi2cx, spl06_data_struct *spl06_data);
static void spl06_read_calibbration(I2C_HandleTypeDef *hi2cx, spl06_data_struct *spl06_data);
static void spl06_calculate_pressure(spl06_data_struct *spl06_data);
static void spl06_calculate_temperature(spl06_data_struct *spl06_data);
static void spl06_moving_average_filter(spl06_data_struct *spl06_data);
static void spl06_pressure_to_altitude(spl06_data_struct *spl06_data);
// uint32_t erroris;
void spl06_init(I2C_HandleTypeDef *hi2cx, spl06_data_struct *spl06_data)
{
   ...
   spl06_write_data(hi2cx, SPL06_PRS_CFG, 0x63, spl06_data);  // ODR 64 OSR 8
   spl06_write_data(hi2cx, SPL06_TMP_CFG, 0x80, spl06_data);  // ODR 1 OSR 0
   spl06_write_data(hi2cx, SPL06_MEAS_CFG, 0x07, spl06_data); // contrinuous mode, pressure and temperature
   spl06_write_data(hi2cx, SPL06_CFG_REG, 0x20, spl06_data);  // active low interrupt, FIFO disable, complete pressure to interrupt
   osDelay(50);
   spl06_read_id(hi2cx, spl06_data);
   spl06_read_calibbration(hi2cx, spl06_data);
...
static void spl06_write_data(I2C_HandleTypeDef *hi2cx, uint8_t reg, uint8_t data, spl06_data_struct *spl06_data)
{
   ...
   // osSemaphoreAcquire(sem_i2c_dma, osWaitForever);
}
...
{
   uint8_t buf[3];
   spl06_read_data(hi2cx, 0x00, buf, 3, spl06_data);
...
{
   uint8_t buf[3];
   spl06_read_data(hi2cx, 0x03, buf, 3, spl06_data);
   int32_t raw = ((int32_t)buf[0] << 16) | ((int32_t)buf[1] << 8) | buf[2];
   if (raw & 0x800000)
      raw |= 0xFF000000;
...
{
   uint8_t buf[18] = {0};
   spl06_read_data(hi2cx, 0x10, buf, 18, spl06_data);
...
{
   spl06_read_pressure_raw(hi2cx, spl06_data); 
   spl06_read_temperature_raw(hi2cx, spl06_data);
   spl06_moving_average_filter(spl06_data);
   spl06_calculate_temperature(spl06_data);
   spl06_calculate_pressure(spl06_data);
   spl06_pressure_to_altitude(spl06_data);
}
...
{
   memset(spl06_data, 0, sizeof(spl06_data_struct));
}