#include "bmi088.h"
static void bmi088_read_id(SPI_HandleTypeDef *hspix, bmi088_sensor_enum sensor);
static void bmi088_write_data(SPI_HandleTypeDef *hspix, bmi088_sensor_enum sensor, uint8_t reg, uint8_t data);
static void bmi088_read_data(SPI_HandleTypeDef *hspix, bmi088_sensor_enum sensor, uint8_t reg, uint8_t *tx_buf, uint8_t *rx_buf, uint16_t size);
static void bmi088_zero_bias(void);
static void bmi088_sensor_data_convert(void);
static bmi088_data_struct bmi088_data;
...
void bmi088_init(SPI_HandleTypeDef *hspix)
{
    ...
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_ACC_SOFTRESET, 0xB6); // rest
    osDelay(1);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_ACC_PWR_CONF, 0x00); // activate mode
    osDelay(1);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_ACC_PWR_CTRL, BMI088_ACC_PWR_CTRL_ON); // enable accelerometer
    osDelay(50);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_ACC_RANGE, BMI088_ACC_RANGE_3G);
    osDelay(1);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_ACC_CONF, 0x8C); // 1600Hz ODR,OSR4
    osDelay(1);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_INT1_IO_CONF, 0x0A); // enable INT1, active high, push-pull
    osDelay(1);
    bmi088_write_data(hspix, BMI088_ACCEL, BMI088_INT1_INT2_MAP_DATA, 0x04); // map INT1 to data ready
    osDelay(1);
    bmi088_read_id(hspix, BMI088_ACCEL);
    osDelay(1); // 等待传感器稳定
     ...
     */
    osDelay(30);
    bmi088_write_data(hspix, BMI088_GYRO, BMI088_GYRO_RANGE, BMI088_GYRO_RANGE_2000DPS);
    osDelay(1);
    bmi088_write_data(hspix, BMI088_GYRO, BMI088_GYRO_BANDWIDTH, 0x01); // ODR 2000hz,OSR 230hz
    osDelay(1);
    ...
    bmi088_read_id(hspix, BMI088_GYRO);
    osDelay(1);
    error_check(ERROR_BMI088, bmi088_data.error);
    /**
...
void bmi088_read_acc_gyro(SPI_HandleTypeDef *hspix)
{
     ...
     */
    bmi088_read_data(hspix, BMI088_ACCEL, BMI088_ACC_X_LSB, tx_buf, rx_buf, 8);
     ...
     *  0x80 is read command, 0x7F is write command
     */
    bmi088_read_data(hspix, BMI088_GYRO, BMI088_GYRO_X_LSB, tx_buf, rx_buf, 7);
...
static void bmi088_sensor_data_convert(void)
{
    ...
    {
        // x y z
        bmi088_data.accl[i] = ((float)bmi088_data.accl_sample[i]) * BMI088_ACC_SENS; // g
    }
    for (uint8_t i = 0; i < 3; i++)
    {
        bmi088_data.gyro[i] = ((float)bmi088_data.gyro_sample[i]) * BMI088_GYRO_SENS; // °/s
    }
}